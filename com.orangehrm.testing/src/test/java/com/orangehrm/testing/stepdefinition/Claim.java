...
import com.orangeHRM.seleniumuiframwork_genricutility.Pages;
import com.orangeHRM.seleniumuiframwork_genricutility.ExcelUtil;
...
public class Claim {
    ...
    public void loadTestData(Scenario scenario) {
        ...
        if (scenario.getName().contains("Submit claim")) {

            event = ExcelUtil.getData(path, "ClaimData", 1, 0);
            currency = ExcelUtil.getData(path, "ClaimData", 1, 1);
            remarks = ExcelUtil.getData(path, "ClaimData", 1, 2);
        }

        // Scenario 2: Search Claim
        if (scenario.getName().contains("Search claim")) {

            refId = ExcelUtil.getData(path, "ClaimData", 1, 3);
        }

        System.out.println("=== Test Data Loaded ===");

        if (event != null) System.out.println("Event: " + event);
        if (currency != null) System.out.println("Currency: " + currency);
        if (remarks != null) System.out.println("Remarks: " + remarks);
        if (refId != null) System.out.println("RefId: " + refId);
    }
    ...
    public void user_selects_event() {
        pages.getSubmitClaim().selectEvent(event);
    }
    ...
    public void claim_should_be_submitted_successfully() {
        ...
        Assert.assertTrue(
            driver.getCurrentUrl().contains("/claim/submitClaim/id/"),
            "Claim not submitted. URL: " + driver.getCurrentUrl()
        );
    }
    ...
    public void url_should_contain_claim_id() {
        ...
        Assert.assertTrue(
            driver.getCurrentUrl().matches(".*/claim/submitClaim/id/\\d+"),
            "URL is not in expected format. URL: " + driver.getCurrentUrl()
        );
    }
}